    ...
    returns (SerializedIssue memory)
{
    ...
    // Convert EnumerableSet to an array of addresses
    address[] memory votersArray = new address[](_issue.voters.length());
    for (uint256 i = 0; i < _issue.voters.length(); i++) {
        votersArray[i] = _issue.voters.at(i);
    }
